...
class Acutum_Truncated_V4(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
        ...
        super(Acutum_Truncated_V4, self).__init__(params, defaults)

    def __setstate__(self, state):
        super(Acutum_Truncated_V4, self).__setstate__(state)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if gm_dot.item() >= 0:
                    exp_avg.add_(grad).mul_(grad_norm.div(grad_norm.add(gm_dot).add(group['eps'])))
                else:
                    coeff = (torch.norm(grad).div(torch.norm(exp_avg).add(group['eps']))).mul(0.5)
